...
if __name__=='__main__':
    ...
    for cohort in cohorts:
        ...
        # Note: in v2, use probes that have no NaN values across all the samples
        beta = pd.read_csv(f'/data/project/3dith/data/450k_xena/{cohort}.HumanMethylation450.tsv', sep = '\t', index_col = 0).dropna()
        beta_merged = beta.merge(meta, left_index = True, right_on = 'name')#CpG probe의 이름을 기준으로 beta value 데이터와 cpg probe의 metadata를 merge.
        ...
        for barcode in tqdm(barcodes, desc=cohort):
            ...
            tmp.to_csv(result_fname, sep = '\t', index = False, header = False)#.bed file로 저장해야 함!!
