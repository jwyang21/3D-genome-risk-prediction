...
def parse_arguments():
    ...
    #/data/project/3dith/pipelines/island-pipeline
    
    args.add_argument('-c', '--cohort', help = 'TCGA cohort', type = str, required = True)
    # 일단 TCGA-BLCA
    
    args.add_argument('--score_fname', help = 'stem-closeness score file name, based on island K-M result', type = str, default = '/data/project/3dith/data/cohort-1-best-score-km.csv', required = True)
...
def get_avg_beta(cohort, cpg_list, S): # get average island or resort CpG beta values (per sample)
...
if __name__=='__main__':
    ...
    # 먼저, 22개 염색체들의 target_cpg (island OR resort CpG probes) 들의 합집합을 구하기 
