...
P_THRESHOLD = 5e-2
SCORE7_DIR = '/data/project/jeewon/research/3D-ITH/pipelines/score2-score4-theta/result/'#TCGA-LIHC/score2-score4-cosine.csv
SCORE7_FNAME = 'score2-score4-cosine.csv'
SCORE7_THRESHOLD = pd.read_csv('/data/project/jeewon/research/3D-ITH/pipelines/downstream-analyses/score7-survival-analysis-threshold.csv', header = None, index_col = 0)
SCORE7_THRESHOLD.columns = ['threshold']
        ...
        if d.shape[0] !=0:
            ...
            if score != 'score5':
                if score != 'score7':
                    ...
                    d['group'] = pd.qcut(d.score7, q = q, labels = list(range(q))) 
            else:# score == 'score5'
...
if __name__=='__main__':
    ...
    if args.score == 'score2':
        ...
        SCORE_FNAME = os.path.join(SCORE7_DIR, cohort, SCORE7_FNAME)
        score7_df = pd.read_csv(SCORE_FNAME, index_col=0)
        original_index = score7_df.index.values
        score7 = score7_df.cos_radian.values.flatten()
        score_df = pd.DataFrame(score7, index = original_index, columns = ['score7']).loc[S].copy()

        tumor_mean, tumor_std, normal_mean, normal_std, tumor_normal_ttest_pval = ttest_plot(score_df, T, N, args.cohort, SAVEDIR, args.score, 'tumor_normal_'+ args.score +'_histogram.png')         

        merged, merged_tumor = merge(clinical_cohort, score_df, T) 

        scaler = MinMaxScaler()

        plt.hist(merged_tumor.score7.values)

        threshold = float(SCORE7_THRESHOLD.loc[args.cohort].values[0])

        raw_merged_tumor = merged_tumor.copy()

        merged_tumor = merged_tumor[merged_tumor['score7'] >= threshold].copy()

        merged_tumor['score7'] = scaler.fit_transform(merged_tumor['score7'].values.reshape(-1,1)).flatten()
        ...
        valid_t, valid_t_pvals, sig_t, sig_t_pvals = survival_analysis(merged_tumor, survival_target, 2, SAVEDIR, 3, 'survival_analysis_'+args.score+'.png', args.cohort, args.score) 
        #Tumor samples only #생존분석 변수 column 전체가 NaN인 경우도 있음. 
