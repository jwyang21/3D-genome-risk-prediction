...
save_dir = f'/data/project/3dith/pipelines/{cpg_type}-pipeline/2_downstream-{cpg_type}/result/repr_vectors/{cohort}'
...
iebdm_N_pc1_files = []
for x in iebdm_pc1_files:
    for n in N:
        if n in x:
            iebdm_N_pc1_files.append(x)
...
for chrom in CHR_LIST:
    ...
    for i in range(len(iebdm_N_pc1_files)):
        current_file = iebdm_N_pc1_files[i]
