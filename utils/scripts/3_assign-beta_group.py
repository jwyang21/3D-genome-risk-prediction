...
for cpg_type in all_cpg_types:
    for i in range(score_files.shape[0]):
        ...
        tumor_mask = np.array([int(x[13:15]) <= 9 for x in avg_beta_df.index.values]) #tumor barcode ends with 01 ~ 09
        ...
        avg_beta_df['group'] = pd.qcut(avg_beta_df.avg_beta, q = num_groups, labels = list(range(num_groups))) #0, 1
