...
def draw_cox_hr(tmp, save_dir, fig_name, cox_version, fig_right=3):
    ...
    for i in range(tmp.shape[0]):
        ...
        x_mins.append(x_min)
        x_maxs.append(x_max)
        #x_maxs.append(float(tmp['coef upper 95%'][covariate]))
    ...
    ax.axvline(x=1, ymin=0, ymax=1, color = 'black', linestyle = '--')#HR = 1
    #x_maxs_max = np.max(x_maxs)
    ...
    for i in range(tmp.shape[0]):    
        ...
        #print(covariate, tmp.coef[covariate])
        str_ = f'HR = {round(hr, 2)} ({round(x_min, 2)}, {round(x_max, 2)}), p = {round(p, 2)}'
        #if cox_version in [4, 5]:
        #    ax.text(5.2, height-0.01, str_, fontsize = 5)
        #else:
        #    ax.text(3.2, height-0.01, str_, fontsize = 5)
...
def draw_log_cox_hr(tmp, save_dir, fig_name, cox_version, fig_right=3):
    ...
    for i in range(len(x_mins)):
        ...
        height = all_heights[i]
        ax.hlines(height, xmin = x_min, xmax = x_max, color='lightgray', linestyle='-', linewidth=2)
    ax.axvline(x=0, ymin=0, ymax=1, color = 'black', linestyle = '--')#HR = 1
    #x_maxs_max = np.max(x_maxs)
...
if __name__=='__main__':
    ...
    for cohort in cohorts:#real #log hazard ratio
        print(f"---\n{cohort}")
    ...
    for cohort in cohorts:#real #hazard ratio
        print(f"---\n{cohort}")
        cohort_dir = os.path.join(result_dir, cohort, f'cox_v{args.cox_version}')
        for f in glob.glob(os.path.join(cohort_dir, f'*.csv')):
            print(f)
