...
if __name__=='__main__':
    ...
    #matching_df = pd.read_csv(args.matching_df_fname, index_col = 0)#여러 cohort 동시에 처리
    ...
    all_pcc_df = pd.DataFrame(np.zeros((len(CHR_LIST), len(matching_df.index.values)), dtype = float), index = CHR_LIST, columns = matching_df.index.values)

    result_dir = f'/data/project/3dith/pipelines/{args.cpg_type}-pipeline/2_downstream-{args.cpg_type}/result/compare-hic-450k-pc1-{args.matrix_type}/tumor-repr'
    ...
    #for chrom in CHR_LIST[:1]:#debug
        ...
        #for i in range(1):#debug
            ...
            repr_basename = []
            for j in range(len(repr_T_vector_pc1_fnames)):
                current_fname = repr_T_vector_pc1_fnames[j]
                current_basename = os.path.basename(current_fname).split('.np')[0]
                repr_basename.append(current_basename)
                print(current_fname)
                repr_T_pc1_10_vectors = np.load(current_fname)
                repr_T_pc1 = np.load(current_fname).mean(axis = 0) #column mean
                assert len(hic_corrmat_pc1) == len(repr_T_pc1)
                #print(pearsonr(hic_corrmat_pc1, repr_T_pc1))

                repr_T_pc1 = standardize(repr_T_pc1)
                pcc = pearsonr(hic_corrmat_pc1, repr_T_pc1)[0]
                pval = pearsonr(hic_corrmat_pc1, repr_T_pc1)[1]
                ...
                for k in range(len(repr_T_pc1_10_vectors)):
                    y3 = repr_T_pc1_10_vectors[k].copy()
                    y3 = standardize(y3)
                ...
                fig_basename = f'repr_T_vector_{chrom}_{str(j)}-w-hic.png'
                fig.tight_layout()
            ...
            current_result_repr.index = repr_basename
