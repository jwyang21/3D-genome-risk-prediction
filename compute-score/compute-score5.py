...
def compute_score(df):
    # (1) 각 score의 역수 취하기
    s2_inv = 1/df.score2.values.flatten()
    s3_inv = 1/df.score3.values.flatten()
    s4_inv = 1/df.score4.values.flatten()
    
    # (2) (1)의 결과의 산술평균 구하기
    tmp = (s2_inv+s3_inv+s4_inv)/3
...
if __name__ == '__main__':
    ...
    npz_fname = os.path.join(SAVEDIR, 'score2345')
    np.savez(npz_fname, rownames = score_df2.index.values, colnames = score_df2.columns.values, score2 = score_df2.score2.values.flatten(),
             score3 = score_df2.score3.values.flatten(), score4 = score_df2.score4.values.flatten(), score5 = score_df2.score5.values.flatten())
    print(npz_fname+'.npz')
    print('----\n')
