...
# ### v2: pcbc ref를 어떤 버전 쓸지 parameterize (all / SC / nonSC)
...
def parse_arguments():
    ...
    parser.add_argument('-r', '--ref_type', help = 'which reference type from pcbc you want to use', type = str, required = True)
...
def import_pc1_ref(chrom, bin_index, ref_type):
    pc1 = np.load(os.path.join(PC1_DIR, 'pcbc', 'ref_'+ref_type+'.npz'), allow_pickle = True)[chrom]
...
if __name__ == '__main__':   
    ...
    for chrom in CHR_LIST:
        ...
        pc1_ref = import_pc1_ref(chrom, pcbc_bins, args.ref_type) #pcbc ref pc1를 import #flattened vector #df (index: bin names, column: pc1)
    ...
    df.to_pickle(os.path.join(SAVEDIR, 'score4_'+args.ref_type+'_euclidean.pickle'))
    print(os.path.join(SAVEDIR, 'score4_'+args.ref_type+'_euclidean.pickle')) # column이 chromosome, row가 해당 cohort의 sample names
